...
|----|---------|----------|----------|
| F1 | Conversational and Multimodal Onboarding | MVP | Onboarding |
| F2 | Learner Profile and Continuous Learner State | MVP | Data Model |
| F3 | Skill-Gap Analysis and Radar Chart | MVP | Intelligence |
| F4 | Recommendation Engine with Multi-Factor Scoring | MVP | Intelligence |
| F5 | Explainable AI (XAI) — "Why This?" | MVP | Trust |
| F6 | Prerequisite-Aware Learning Path Generation | MVP | Intelligence |
| F7 | Next Best Action (NBA) Engine | MVP | Intelligence |
| F8 | Progress Tracking and Manual Feedback | MVP | Interaction |
| F9 | Assessment-Driven Adaptive Recalculation | MVP | Intelligence |
| F10 | AI Mentor (Contextual Agentic Chatbot) | Should Have | AI Agent |
| F11 | Dashboard with Progress and Skill Visualization | MVP | UI/UX |
| F12 | Resume-to-JD Pipeline | Should Have | Onboarding |
...
- The LLM extracts structured data using forced JSON output: `{ goal, current_skills[], target_role, time_budget_hours, deadline_months, experience_level }`
...
- Clicking it reveals a natural-language explanation generated by the LLM, grounded in the scoring system's actual factors
- Example: *"Your goal requires Advanced Pandas, but you currently have Basic Python. This course was selected because it bridges that exact gap (skill relevance: 92%), matches your intermediate difficulty level, and fits your 2-hour/week budget."*
...
### F10 — AI Mentor (Contextual Agentic Chatbot)

**Purpose:** Provide contextual help, not generic answers. Execute scoped actions via tool-calling.
...
**Tool Functions (LangChain/LangGraph ReAct Agent):**

| Tool | Action | Safety |
|------|--------|--------|
| `get_progress()` | Retrieve current progress | Read-only |
| `get_skill_gap()` | Retrieve skill-gap analysis | Read-only |
| `get_recommendations()` | Get current recommendations with explanations | Read-only |
| `mark_module_complete()` | Mark a module as completed | Requires confirmation |
| `skip_module()` | Skip a module, adjust roadmap | Requires confirmation |
| `trigger_recalculation()` | Full adaptive recalculation | Requires confirmation |

**Safety:** State-changing actions always ask the learner for confirmation before executing.

**Difference from a generic chatbot:** It reasons over structured learner data using tool-calling, not just generating text.
